...
export function renderAgents(main, ctx) {
  ...
  main.innerHTML = `
    ...
    ${
      agents.length
        ? `<div class="cc-card-grid">${agents.map((a) => `<div class="cc-stat-tile"><div class="cc-stat-label">Agent</div><div>${escapeHtml(a.name || a.id)}</div></div>`).join("")}</div>`
        : emptyState(
    ...
    ${
      ...
      <div class="cc-section">
        ...
        <div class="cc-card-grid">
          <div class="cc-stat-tile"><div class="cc-stat-label">Runs recorded</div><div class="cc-stat-value">${SAMPLE_AGENT_RUNS.runs_recorded}</div></div>
          <div class="cc-stat-tile"><div class="cc-stat-label">Success rate</div><div class="cc-stat-value">${Math.round(SAMPLE_AGENT_RUNS.success_rate * 100)}%</div></div>
          <div class="cc-stat-tile"><div class="cc-stat-label">Last run</div><div class="cc-stat-value" style="font-size:16px;">${new Date(SAMPLE_AGENT_RUNS.last_run).toLocaleString()}</div></div>
        </div>
